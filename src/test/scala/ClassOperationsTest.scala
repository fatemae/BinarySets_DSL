...
class ClassOperationsTest extends AnyFlatSpec with Matchers{
  ...
  it should "should result in creating an object of ClassA" in {
    ...
    val str = "HashMap(<dynamicType> -> ClassA1, <staticType> -> ClassA1, <method> -> HashMap(ClassA1+0 -> HashMap(<methodBody> -> ArraySeq(Assign(setA,Value(HashSet(2)))), <methodParams> -> HashMap())), <field> -> HashMap(setA -> HashSet(2)))"
    assert(str==ObjectType("newObj1").eval().toString)
  }
  ...
  it should "should result in creating a method with no params in a class and invoking the method on the class object" in {
    ...
    val op = InvokeMethod(ObjectType("newObj2"), "newMethodNoParam", Parameters(null))
  ...
  it should "should result in creating a parameterized method and constructor in a class and invoking the method on the class object" in {
    ...
    val op = InvokeMethod(ObjectType("newObj3"), "newMethodWithParam", Parameters(collection.mutable.Map("x"->1, "y"->2)))
  ...
  it should "result in inheritance of ClassB to ClassC" in {
    ...
    val op = InvokeMethod(ObjectType("newObjB"), "newMethod" , Parameters(collection.mutable.Map("x"->1, "y"->2)))
